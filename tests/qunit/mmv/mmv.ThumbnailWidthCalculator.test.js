...
QUnit.test( 'We get sensible image sizes when we ask for them', ( assert ) => {
	...
	assert.strictEqual( twc.findNextBucket( 480 ), 500, 'Low target size gives us lowest possible size bucket' );
	assert.strictEqual( twc.findNextBucket( 500 ), 500, 'Asking for a bucket size gives us exactly that bucket size' );
	assert.strictEqual( twc.findNextBucket( 500.00001 ), 960, 'Asking for greater than an image bucket definitely gives us the next size up' );
	assert.strictEqual( twc.findNextBucket( 1800 ), 1920, 'The image bucketing also works on big screens' );
	assert.strictEqual( twc.findNextBucket( 3000 ), 3840, 'The image bucketing also works on REALLY big screens' );
} );
